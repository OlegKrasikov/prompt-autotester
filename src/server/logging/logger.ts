...
export function getLogger(req?: NextRequest | { headers: Headers }) {
  ...
  return {
    info: (message: string, meta?: Record<string, unknown>) => baseLog("info", message, withRid(meta)),
    warn: (message: string, meta?: Record<string, unknown>) => baseLog("warn", message, withRid(meta)),
    error: (message: string, meta?: Record<string, unknown>) => baseLog("error", message, withRid(meta)),
  };
}
