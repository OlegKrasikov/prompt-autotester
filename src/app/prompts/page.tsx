  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const msg = typeof errorData.error === 'string' ? errorData.error : (errorData.error?.userMessage || errorData.error?.message || 'Failed to delete prompt');
  ...
  const handleDuplicate = async (id: string) => {
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json().catch(() => ({}));
        const msg = typeof errorData.error === 'string' ? errorData.error : (errorData.error?.userMessage || errorData.error?.message || 'Failed to duplicate prompt');
          ...
          <CardContent padding="lg">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <Select
                ...
                onChange={(e) => setFilters(prev => ({ ...prev, status: (e.target.value as PromptStatus) || undefined }))}
              >
            ...
            <CardContent>
              <div className="text-center py-12">
                ...
                <p className="text-sm text-[color:var(--color-muted-foreground)] mb-6 max-w-sm mx-auto">
                  Create your first prompt to start building your collection for prompt testing and comparison.
                </p>
                <button 
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-[color:var(--color-surface-1)] border-b border-[color:var(--color-divider)]">
                    <tr>
                      <th className="px-3 py-3 xl:px-4 xl:py-4 2xl:px-6 2xl:py-4 text-left text-xs font-semibold text-[color:var(--color-foreground)] uppercase tracking-wider">Name</th>
                      <th className="px-3 py-3 xl:px-4 xl:py-4 2xl:px-6 2xl:py-4 text-left text-xs font-semibold text-[color:var(--color-foreground)] uppercase tracking-wider hidden xl:table-cell">Tags</th>
                      <th className="px-3 py-3 xl:px-4 xl:py-4 2xl:px-6 2xl:py-4 text-left text-xs font-semibold text-[color:var(--color-foreground)] uppercase tracking-wider w-20">Status</th>
                      <th className="px-3 py-3 xl:px-4 xl:py-4 2xl:px-6 2xl:py-4 text-left text-xs font-semibold text-[color:var(--color-foreground)] uppercase tracking-wider hidden lg:table-cell w-28">Updated</th>
                      <th className="px-3 py-3 xl:px-4 xl:py-4 2xl:px-6 2xl:py-4 text-right text-xs font-semibold text-[color:var(--color-foreground)] uppercase tracking-wider w-32">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[color:var(--color-divider)]">
                    {prompts.map((prompt) => (
                      <tr key={prompt.id} className="hover:bg-[color:var(--color-surface-1)] transition-colors">
                        <td className="px-3 py-3 xl:px-4 xl:py-4 2xl:px-6 2xl:py-4">
                          <div className="flex flex-col">
                            <span className="text-sm font-medium text-[color:var(--color-foreground)] truncate">
                              {prompt.name}
                            </span>
                            {prompt.description && (
                        ...
                        <td className="px-3 py-3 xl:px-4 xl:py-4 2xl:px-6 2xl:py-4 hidden xl:table-cell">
                        ...
                        <td className="px-3 py-3 xl:px-4 xl:py-4 2xl:px-6 2xl:py-4 hidden lg:table-cell">
                        ...
                        <td className="px-3 py-3 xl:px-4 xl:py-4 2xl:px-6 2xl:py-4">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            >
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="xl:mr-1">
                                <path d="M12 22h6a2 2 0 0 0 2-2V7l-5-5H6a2 2 0 0 0-2 2v10"/>
                                <path d="M14 2v4a2 2 0 0 0 2 2h4"/>
                                <path d="M10.4 12.6a2 2 0 1 1 3 3L8 21l-4 1 1-4Z"/>
                              </svg>
