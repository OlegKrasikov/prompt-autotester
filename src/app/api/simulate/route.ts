      ...
      try {
        ...
        if (hasPrimary) {
          try {
            ...
            console.warn('API key re-encryption (rotation) failed; proceeding with legacy-decrypted key.');
          }
        }
...
async function* streamSimulation(
  ...
  for (const turn of scenarioTurns) {
    if (turn.turnType === 'USER' && turn.userText) {
      ...
      try {
        ...
        const errorMessage: ChatMessage = { role: "assistant", content: "[Error: Failed to get AI response]" };
  ...
  const modelDisplay = options.length > 0 ? `${modelConfig.model} (${options.join(", ")})` : modelConfig.model;
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!parsed.success) {
      return errorJson('Invalid simulate payload', { status: 400, details: parsed.error.flatten() });
    }
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          const currentStream = streamSimulation(body.oldPrompt, scenarioTurns, modelConfig, apiKey, 'current', user.id);
          const editedStream = streamSimulation(body.newPrompt, scenarioTurns, modelConfig, apiKey, 'edited', user.id);
