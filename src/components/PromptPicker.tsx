...
export function PromptPicker({ value, onChange, error }: PromptPickerProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {loading ? (
        <div className="flex items-center gap-2 p-3 border border-[color:var(--color-border)] rounded-[var(--radius)] bg-[color:var(--color-surface)]">
          <Spinner size="sm" />
        ...
        <>
          ...
          >
            ...
            {prompts.length === 0 ? (
              <option value="" disabled>No published prompts available</option>
            ) : (
              prompts.map((prompt) => (
          ...
          {prompts.length === 0 && !loading && (
            <div className="p-3 rounded-[var(--radius)] text-sm bg-[color:var(--color-warning)]/10 text-[color:var(--color-warning)] border border-[color:var(--color-warning)]/20">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="text-xs mt-1">Create and publish prompts to use them for testing.</div>
                </div>
              </div>
            </div>
          )}
