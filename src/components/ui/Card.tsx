...
const Card: React.FC<CardProps> = ({
  ...
  const baseClasses =
    'bg-[color:var(--color-surface)] rounded-[var(--radius-md)] transition-all duration-200';
...
const CardHeader: React.FC<{
  ...
  <div
    className={`border-b border-[color:var(--color-divider)] pb-3 ${className || ''}`}
    onClick={onClick}
  >
    {children}
  </div>
...
const CardTitle: React.FC<{
  ...
  <h3
    className={`3xl:text-3xl 4xl:text-4xl text-lg font-semibold text-[color:var(--color-foreground)] xl:text-xl 2xl:text-2xl ${className || ''}`}
  >
    {children}
  </h3>
);
...
const CardFooter: React.FC<{
  ...
  <div className={`border-t border-[color:var(--color-divider)] pt-3 ${className || ''}`}>
    {children}
  </div>
);
