  ...
  const getIconAndColors = () => {
    switch (type) {
      case 'success':
        return {
          icon: (
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-label="Success">
      ...
      case 'warning':
        return {
          icon: (
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-label="Warning">
      ...
      case 'error':
        return {
          icon: (
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-label="Error">
              <circle cx="12" cy="12" r="10"/>
      ...
      default: // info
        return {
          icon: (
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-label="Information">
              <circle cx="12" cy="12" r="10"/>
              <path d="M12 16v-4"/>
              <path d="M12 8h.01"/>
            </svg>
    ...
    >
      <ModalContent>
        <div className="space-y-4">
          ...
          <div 
            className={`p-4 rounded-[var(--radius)] ${bgColor} border ${borderColor}`}
            role="alert"
            aria-live="polite"
          >
            <div className="flex items-start gap-3">
              <div className={`w-5 h-5 mt-0.5 ${iconColor}`} role="img" aria-label={ariaLabel}>
                {icon}
              </div>
              <div className={textColor}>
                {formatCompactMessage(message)}
              </div>
            </div>
          </div>
        </div>
