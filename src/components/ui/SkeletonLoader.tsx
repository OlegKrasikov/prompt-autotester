...
const SkeletonCard: React.FC<{ className?: string }> = ({ className }) => (
  <div className={`p-4 border border-[color:var(--color-border)] rounded-[var(--radius-md)] bg-[color:var(--color-surface)] ${className || ""}`}>
...
const SkeletonTable: React.FC<{ rows?: number; cols?: number; className?: string }> = ({ 
  ...
  <div className={`border border-[color:var(--color-border)] rounded-[var(--radius-md)] overflow-hidden bg-[color:var(--color-surface)] ${className || ""}`}>
    {/* Header */}
    <div className="p-4 border-b border-[color:var(--color-border)] bg-[color:var(--color-surface-1)]">
    ...
    {Array.from({ length: rows }).map((_, rowIndex) => (
      <div key={rowIndex} className="p-4 border-b border-[color:var(--color-border)] last:border-b-0">
        <div className="grid gap-4" style={{ gridTemplateColumns: `repeat(${cols}, 1fr)` }}>
