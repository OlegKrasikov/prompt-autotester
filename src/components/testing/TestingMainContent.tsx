  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 sm:gap-3 text-xl sm:text-2xl font-bold text-[color:var(--color-foreground)] mb-1 sm:mb-2">
            <div className="w-6 h-6 sm:w-8 sm:h-8 bg-[color:var(--color-accent)] rounded-[var(--radius)] flex items-center justify-center">
              <svg width="14" height="14" className="sm:w-[18px] sm:h-[18px]" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <p className="text-sm sm:text-base text-[color:var(--color-muted-foreground)] hidden sm:block">
      ...
      {/* Error Display */}
      {error && (
        <Card variant="outlined" className="border-[color:var(--color-danger)] bg-[color:var(--color-danger-light)]/5">
        ...
        <Card variant="elevated" className="lg:col-span-1">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="border-t border-[color:var(--color-border)] pt-4">
              <div className="text-sm font-medium text-[color:var(--color-foreground)] mb-3">Settings</div>
            ...
            {/* Status Message */}
            {statusMessage && (
              <div className="p-3 rounded-[var(--radius)] bg-[color:var(--color-warning)]/10 border border-[color:var(--color-warning)]/20">
                <div className="flex items-center gap-2">
        ...
        <CardContent>
          {promptsLoading ? (
            <div className="flex items-center justify-center gap-3 py-8">
              <Spinner size="sm" />
              <span className="text-sm text-[color:var(--color-muted-foreground)]">Loading prompts...</span>
            </div>
          ) : !hasPrompts ? (
            <div className="text-center py-8">
              <div className="w-16 h-16 rounded-[var(--radius)] flex items-center justify-center mx-auto mb-4">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[color:var(--color-muted-foreground)]">
                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                  <polyline points="14,2 14,8 20,8"/>
                  <line x1="16" y1="13" x2="8" y2="13"/>
                  <line x1="16" y1="17" x2="8" y2="17"/>
              ...
              <p className="text-sm text-[color:var(--color-muted-foreground)] mb-4 max-w-md mx-auto">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="text-xs text-[color:var(--color-muted-foreground)]">
                  Select a published prompt to automatically fill the &quot;Current prompt&quot; field, or type your own prompt below.
                </div>
              </div>
              ...
              {selectedPromptId && (
                ...
                >
                  ...
                  <div className="flex flex-col gap-2 w-full touch-pan-y" role="region" aria-label="Current prompt view">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-medium text-[color:var(--color-foreground)] tracking-wide uppercase">
                        Current prompt
                      </label>
                      <div className="flex items-center gap-1 sm:gap-2 text-xs text-[color:var(--color-muted-foreground)]">
                        ...
                        {oldPrompt.split('\n').length > 1 && (
                          <>
                            <span>•</span>
                            <span className="hidden sm:inline">{oldPrompt.split('\n').length} lines</span>
                    ...
                    <div
                      className="w-full h-[200px] p-2 sm:p-3 text-sm bg-[color:var(--color-surface)] border rounded-[var(--radius)] border-[color:var(--color-border)] font-mono leading-relaxed overflow-auto whitespace-pre-wrap touch-pan-y scrollbar-thin scrollbar-thumb-[color:var(--color-border)] scrollbar-track-transparent hover:scrollbar-thumb-[color:var(--color-accent)]/50"
                    ...
                    <div className="flex justify-end items-center gap-2 text-xs text-[color:var(--color-muted-foreground)]">
                      {oldPrompt.trim() && (
                        <>
                          <span className="hidden sm:inline">{oldPrompt.trim().split(/\s+/).length} words</span>
                          <span className="sm:hidden">{oldPrompt.trim().split(/\s+/).length}w</span>
                          <div className={`w-2 h-2 rounded-full ${
                            oldPrompt.length > 2000 ? 'bg-[color:var(--color-danger)]' : 
                            oldPrompt.length > 1000 ? 'bg-[color:var(--color-warning)]' : 
                            'bg-[color:var(--color-success)]'
                          }`} title={
                            oldPrompt.length > 2000 ? 'Very long prompt' :
                            oldPrompt.length > 1000 ? 'Long prompt' :
                            'Good length'
                          } />
                        </>
                      )}
                  ...
                  <div className="hidden 3xl:flex flex-col gap-2 w-full" role="region" aria-label="Prompt preview">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-medium text-[color:var(--color-foreground)] tracking-wide uppercase">
                        Preview
                      </label>
                    ...
                    <div
                      className="w-full h-[200px] p-3 text-sm bg-[color:var(--color-surface)] border rounded-[var(--radius)] border-[color:var(--color-border)] font-mono leading-relaxed overflow-auto whitespace-pre-wrap"
                    >
                      {newPrompt ? (
                  ...
                  <div className="hidden 4xl:flex flex-col gap-2 w-full" role="region" aria-label="Prompt metadata">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-medium text-[color:var(--color-foreground)] tracking-wide uppercase">
                        Metadata
                      </label>
                    ...
                    <div className="space-y-3 p-3 bg-[color:var(--color-surface)] border rounded-[var(--radius)] border-[color:var(--color-border)]">
                      <div className="text-xs space-y-2">
                        <div className="flex justify-between">
                          <span className="text-[color:var(--color-muted-foreground)]">Length change:</span>
                        ...
                        <div className="flex justify-between">
                          <span className="text-[color:var(--color-muted-foreground)]">Word change:</span>
                          <span className={`font-medium ${
                            newPrompt.trim().split(/\s+/).length > oldPrompt.trim().split(/\s+/).length 
                              ? 'text-[color:var(--color-warning)]' 
                              : newPrompt.trim().split(/\s+/).length < oldPrompt.trim().split(/\s+/).length 
                                ? 'text-[color:var(--color-success)]'
                                : 'text-[color:var(--color-muted-foreground)]'
                            ...
                            {newPrompt.trim().split(/\s+/).length > oldPrompt.trim().split(/\s+/).length ? '+' : ''}
                            {newPrompt.trim().split(/\s+/).length - oldPrompt.trim().split(/\s+/).length} words
                          </span>
                        </div>
                        
                        <div className="flex justify-between">
                          <span className="text-[color:var(--color-muted-foreground)]">Complexity:</span>
                          <span className="font-medium text-[color:var(--color-foreground)]">
                            {newPrompt.split('\n').length > 10 ? 'High' : 
                             newPrompt.split('\n').length > 5 ? 'Medium' : 'Simple'}
                          </span>
                        </div>
                        
                        <div className="flex justify-between">
                          <span className="text-[color:var(--color-muted-foreground)]">Variables:</span>
                          <span className="font-medium text-[color:var(--color-foreground)]">
